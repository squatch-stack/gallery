...
Applicant / recipient: {{APPLICANT}}. Vendor: {{VENDOR}}.
Program: {{PROGRAM}}. Location: {{LOCATION}}.
Purpose and intended public benefit: {{PURPOSE}}

The vendor will document the subjects below for public interpretation, using
{{DAYS_ON_SITE}} on-site day(s), up to {{DAY_HOURS}} hours per day, under the
{{TIER}} tier. Boundaries and accessible faces must be confirmed at kickoff.
...
Mesh option: **{{MESH_SCOPE}}**. If selected, add a textured OBJ with materials
...
Dataset publication: **{{DATASET_SCOPE}}**. If included, prepare the approved
archive and metadata, deposit with {{REPOSITORY}}, and deliver the repository
...
Hosting: {{HOSTING_MONTHS}} month(s) from acceptance, then handoff or approved renewal.
...
Accessibility pass: **{{ACCESS_SCOPE}}**. When included, the delivered viewer and
...
|---|---|
| Kickoff and permission review | {{START_DATE}}; applicant and vendor agree boundaries, rights and repository |
| Field capture | {{DAYS_ON_SITE}} day(s), scheduled only after written access confirmation |
| Draft delivery and evidence | Within {{DRAFT_DAYS}} business days after capture and receipt of applicant content |
| Consolidated review | Applicant supplies acceptance or a defect list within {{REVIEW_BUSINESS_DAYS}} business days |
| Correction and retest | Vendor allows {{REVISION_BUSINESS_DAYS}} business days after consolidated review |
...
handoff archive for {{ARCHIVE_RETENTION_MONTHS}} months after acceptance; applicant
...
Proposed contract allocation: upon payment, vendor assigns to {{APPLICANT}} its
...
Attached estimate: gross scope value **{{GROSS_TOTAL}}**, proposed donated capture
**{{IN_KIND}}**, total cash payable **{{CASH_TOTAL}}**. No donation commitment exists
