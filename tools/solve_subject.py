...
subject = Path(args[0]).expanduser()
images = subject / "images"
db = subject / "database.db"
sparse = subject / flag("--out", "sparse")
...
if not flag("--remap"):
    ...
    sift.max_image_size = flag("--image-size", 2400, int)
    sift.sift.max_num_features = flag("--features", 8192, int)
    ...
    matching.guided_matching = bool(flag("--guided"))
    if flag("--sequential"):
...
if flag("--global"):
    ...
    how = "relaxed incremental" if flag("--relaxed") else "incremental"
